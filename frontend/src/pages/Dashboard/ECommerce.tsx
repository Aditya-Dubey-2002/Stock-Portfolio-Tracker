...
const ECommerce: React.FC = () => {
  ...
  const [realizedProfit,setRealizedProfit] = useState<number|any>('');
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        setRealizedProfit(response.data.userDetails.profit);
        // console.log(response.data);
        // console.log(response) // Store the user details in state
      } catch (err: any) {
        console.error('Error fetching profile:', err);
        setProfileError(err.response?.data?.error || 'Failed to fetch user profile');
      } finally {
        setProfileLoading(false);
      }
    };
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        ...
        <CardDataStats title="Unrealized Profit/Loss" total={loading ? <Loader /> : "$" + totalProfitLoss} rate={loading ? <Loader /> : netPct + "%"} levelDown={!loading && parseFloat(netPct) < 0}
          levelUp={!loading && parseFloat(netPct) >= 0}>
          <svg
            className="fill-primary dark:fill-white"
            width="22"
        ...
        <CardDataStats title="Realized Profit/Loss" total={profileLoading||loading ? <Loader /> : "$" + realizedProfit} >
          <svg
            className="fill-primary dark:fill-white"
            width="22"
            height="22"
